...
func (p *currentProjection) match(entity *spec.Column) (*spec.Column, error) {
	...
	for _, current := range p.current.Columns {
		...
		currentSource := currentEntitySource(current)
...
func (p *currentProjection) field(column *spec.Column) plan.FieldRef {
	if column == nil {
		return plan.FieldRef{}
	}
	source := strings.TrimSpace(column.Source)
	if strings.TrimSpace(column.Expression) != "" {
		source = currentEntitySource(column)
	}
	return plan.FieldRef{Field: typecatalog.FieldName(column.Name), Source: source, Type: column.EffectiveType()}
}

func currentEntitySource(column *spec.Column) string {
	if column == nil {
		return ""
	}
	if source := strings.TrimSpace(column.Expression); source != "" {
		return source
	}
