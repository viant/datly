...
` + strings.Replace(strings.TrimPrefix(DQL, PackageDirective+"\n"), "SELECT o.*, Items.*, Kinds.*,", "SELECT o.*, Items.*, Kinds.*, type(o,'rows.Order'), dest(o,'order.go'), type(Items,'items.Item'), dest(Items,'item.go'), lifecycle_type(o,'rh.Hooks'),lifecycle_type(Items,'ch.Hooks'),", 1)
