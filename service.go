...
func (s *Service) NewComponentSession(aComponent *repository.Component, opts ...SessionOption) *session.Session {
	...
	aSession := session.New(aComponent.View, session.WithLocatorOptions(options...), session.WithStateResource(sessionOpt.resource), session.WithOperate(s.operator.Operate))
	return aSession
}
