...
func TestRouter(t *testing.T) {
	...
	testcases := []*testcase{
		...
		},
		{
			...
			expected:    `{"Errors":[{"Param":"RequestBody","Message":"Key: 'Id' Error:GetField validation for 'Id' failed on the 'required' tag","Object":[{"Value":0,"GetField":"Id","Reason":"required"}]}]}`,
		},
		{
		...
		},
		{
			...
			visitors: map[string]plugins.BasicCodec{
				"AsStrings": plugins.NewCodec("AsStrings", &asStrings{}, reflect.TypeOf([]string{})),
			},
		...
		},
		{
			...
			expectedHeaders: map[string][]string{
				"Datly-Metrics-Event_types": {
					`{"View":"event_types","Template":[{"SQL":"SELECT  COALESCE(id,0) AS id,  COALESCE(type,\"\") AS type,  COALESCE(code,\"\") AS code FROM (SELECT * FROM EVENT_TYPES   ) AS t ","CacheStats":{"Type":"lazy","RecordsCounter":1,"Key":"-7975909061465053937","FoundLazy":true}}],"TemplateMeta":[{"SQL":"SELECT COUNT(*) AS total_count FROM (SELECT  COALESCE(id,0) AS id,  COALESCE(type,\"\") AS type,  COALESCE(code,\"\") AS code FROM (SELECT * FROM EVENT_TYPES   ) AS t ) T","CacheStats":{"Type":"lazy","RecordsCounter":1,"Key":"4341218873677092370","FoundLazy":true}}],"Elapsed":"5ms"}`,
					`{"View":"event_types","Template":[{"SQL":"SELECT  COALESCE(id,0) AS id,  COALESCE(type,\"\") AS type,  COALESCE(code,\"\") AS code FROM (SELECT * FROM EVENT_TYPES   ) AS t ","CacheStats":{"Type":"write","RecordsCounter":0,"Key":""}}],"TemplateMeta":[{"SQL":"SELECT COUNT(*) AS total_count FROM (SELECT  COALESCE(id,0) AS id,  COALESCE(type,\"\") AS type,  COALESCE(code,\"\") AS code FROM (SELECT * FROM EVENT_TYPES   ) AS t ) T","CacheStats":{"Type":"write","RecordsCounter":0,"Key":""}}],"Elapsed":"5ms"}`,
				},
				"Datly-Metrics-Events": {`{"View":"events","Template":[{"SQL":"SELECT  COALESCE(id,0) AS id,  t.timestamp,  COALESCE(event_type_id,0) AS event_type_id,  COALESCE(quantity,0) AS quantity,  COALESCE(user_id,0) AS user_id FROM (SELECT * FROM EVENTS WHERE 1 = 1 AND EVENT_TYPE_ID IN (?, ?, ?, ? )  AND ( event_type_id IN (?, ?, ?, ?))  ) AS t ","Args":[1,2,11,111,1,2,11,111],"CacheStats":{"Type":"warmup","RecordsCounter":4,"Key":"event_type_id#-4347759850797876299","FoundWarmup":true}}],"TemplateMeta":[{"SQL":"SELECT event_type_id, COUNT(*) AS total_count FROM (SELECT  COALESCE(id,0) AS id,  t.timestamp,  COALESCE(event_type_id,0) AS event_type_id,  COALESCE(quantity,0) AS quantity,  COALESCE(user_id,0) AS user_id FROM (SELECT * FROM EVENTS WHERE 1 = 1 AND EVENT_TYPE_ID IN (?, ?, ?, ? )  AND ( event_type_id IN (?, ?, ?, ?))  ) AS t ) T GROUP BY event_type_id","Args":[1,2,11,111,1,2,11,111],"CacheStats":{"Type":"warmup","RecordsCounter":4,"Key":"event_type_id#2930711288512290438","FoundWarmup":true}}],"Elapsed":"5ms"}`},
			},
