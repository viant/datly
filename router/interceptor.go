...
func (i *RouteInterceptor) init(URL string) error {
	...
	evaluator, err := expand.NewEvaluator(nil, nil, nil, i.Template, i.lookupType, expand.WithCustomContexts(i.contextType))
	if err != nil {
		return err
	}
...
func (i *RouteInterceptor) Evaluate(request *http.Request) (*IntereceptorState, error) {
	...
	state, err := i.evaluator.Evaluate(nil, expand.WithCustomContext(i.newContext(*ctx)))
