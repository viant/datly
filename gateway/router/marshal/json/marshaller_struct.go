...
func (s *structMarshaller) MarshallObject(ptr unsafe.Pointer, sb *MarshallSession) error {
	...
	for _, stringifier := range s.marshallers {
		...
		if stringifier.indirectXField != nil {
			...
			if !stringifier.omitEmpty {
				holderJsonTag, ok := stringifier.indirectXField.Tag.Lookup("json")
				if ok && strings.Contains(holderJsonTag, "omitempty") {
		...
		if stringifier.omitEmpty && (isZeroVal || isNil) {
			continue
		}
		if prevLen != sb.Len() {
			sb.WriteByte(',')
