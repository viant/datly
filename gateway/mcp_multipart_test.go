...
type mcpHybridBody struct {
	Data []string `json:"data,omitempty"`
}

func (b *mcpHybridBody) UnmarshalXLS(context.Context, []byte) error { return nil }
...
func TestMCPHybridBodySchemaOffersEitherJSONOrRawBlob(t *testing.T) {
	bodyParameter := state.NewParameter("Body", state.NewBodyLocation("body"), state.WithParameterSchema(state.NewSchema(reflect.TypeOf(&mcpHybridBody{}))))
	component := &repository.Component{Contract: contract.Contract{Input: contract.Input{Type: state.Type{Parameters: []*state.Parameter{bodyParameter}}}}}
	inputType := (&Router{}).buildToolInputTypeForPath(component, nil)
	toolSchema := schema.ToolInputSchema{}
	require.NoError(t, toolSchema.Load(reflect.New(inputType).Interface()))

	assert.Contains(t, toolSchema.Properties, "Body")
	assert.Contains(t, toolSchema.Properties, "Blob")
	assert.NotContains(t, toolSchema.Required, "Body")
	assert.NotContains(t, toolSchema.Required, "Blob")
}

func TestMCPHybridBodyRawBlobUsesBinaryContentType(t *testing.T) {
	bodyParameter := state.NewParameter("Body", state.NewBodyLocation("body"), state.WithParameterSchema(state.NewSchema(reflect.TypeOf(&mcpHybridBody{}))))
	component := &repository.Component{Contract: contract.Contract{Input: contract.Input{Type: state.Type{Parameters: []*state.Parameter{bodyParameter}}}}}
	body, rpcErr := buildMCPRawBlobBody(component, map[string]interface{}{
		"Blob": map[string]interface{}{
			"data":     base64.StdEncoding.EncodeToString([]byte("xlsx bytes")),
			"filename": "campaign.xlsx",
			"mimeType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
		},
	})
	...
	request, requestErr := (&Router{}).newToolHTTPRequest(context.Background(), "PATCH", "http://localhost/bulkimport/write", body)
	require.Nil(t, requestErr)
	assert.Equal(t, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", request.Header.Get("Content-Type"))
	content, err := io.ReadAll(request.Body)
	require.NoError(t, err)
	assert.Equal(t, "xlsx bytes", string(content))
}
