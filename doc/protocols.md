...
[All guides](README.md) · [Security](security.md)
...
[custom handlers](custom-handlers.md) for bytes and [API documentation](api-documentation.md)
...
The [MCP server API](../mcp/server/config.go) supports stdio, SSE and streamable
...
[Custom composition](custom-handlers.md) explains typed forwarding and the
...
and canonical control inputs; see [async](async.md). [Warmup HTTP](cache-and-warmup.md) requires an explicit
...
plus exact declared product imports. [Packaging](authoring-skills.md) explains
...
See the [developer service guide](../mcp/developer/README.md) for all seven tools, per-tool authority and the parent-approved authoring-root reporting correction.
