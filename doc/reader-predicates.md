...
`TestReaderPredicateGroupsDQLSQLite` loads the complete DQL above through the
native compiler, uses linked typed contracts and the invocation binder, then runs
the real reader against the shared SQLite harness. It covers WHERE, an existing
WHERE plus AND, fixed scope plus a completely empty optional Builder, chained and
nested calls, and repeated parameters across groups. It checks row identity and scope, marker
presence, SQL placeholders and ordered arguments, including a quoted SQL-like
search value. `TestBuilderGroupComposition` covers empty rendering and mixed
operator/parenthesis behavior. No custom production parser or binder is needed.

`TestReaderCustomAndBuiltinPredicateGroupsDQLSQLite` verifies both WHERE and AND with declared verified JWT input, imported handler aliases, scoped injection, mixed built-in/custom groups, optional dependency skipping, nil/empty criteria, zero bounds, quoted values, wrong signatures, expiry and business/binding errors. Existing `TestCustomPredicateBindsComponentResultSQLite` supplies component-result injection evidence; `TestCustomPredicateScopedSQLite` includes cancellation.
