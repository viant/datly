...
explicit authorization; see [configured async](../standalone/ASYNC.md).
...
The [current API](../application/async.go) defines the exact structs; see the
[configuration guide](configuration.md) for deployment settings. The watcher starts on the first successful publication.
...
[configuration and host contract](../standalone/ASYNC.md) describes defaults,
...
[JWT input pattern](security.md) before connecting a transport submit path.
...
The [SQLite schema fixture](../internal/testharness/sqlite/jobs_schema.go) contains
