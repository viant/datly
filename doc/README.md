...
Both autonomous and custom mode datly can be deploy as standalone app or as Docker, Kubernetes Cloud Serverless runtimes (lambda,GCF,Cloud Run).
...
#if($FoosDBRecordsIndex.HasKey($recFoos.Id) == false) $logger.Fatal("not found record with %v id", $recFoos.Id) #end
