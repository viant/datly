...
See [cache and warmup](product/datly/doc/cache-and-warmup.md).
...
a child collection as root after selection. Read [selectors and formats](product/datly/doc/selectors-and-formats.md)
...
- [Async](product/llm/datly-writer/references/mutation-messages.md#async-and-dry-run): original 34-column job schema, AFS events, canonical replay/current authorization, explicit HTTP controls, cache-only result inspection and limited reader dryrun.
- [Observability](product/datly/doc/observability.md): native capture plus optional default-off bounded async OTel export; no zero-cost claim.
- [API documentation](product/datly/doc/api-documentation.md): `$DocGlobalURLs`, `$DocURL`, `$DocURLs`, `$DocBaseURL`; global then rule YAML, explicit annotation precedence and shared embedded OpenAPI/MCP schemas.
- [Static content](product/datly/doc/static-content.md): `$static_resource('site','public')` or `$static_content('content-url','root')`, resource manifests, explicit filesystem authority and CORS.
