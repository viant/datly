...
| Async | Scoped rebase approval covers capture/replay/results/differ, current authorization and completion ownership. Standalone Jobs/AFS/watch/HTTP configuration uses a mandatory linked host authorizer; absent policy rejects startup. See [configured async](references/product/datly/standalone/ASYNC.md) for settings and authorization. Complete standalone/config race tests pass, including network tests. |
