...
- For project init/build and deployment, read [project-build.md](references/product/llm/datly-custom-component/references/project-build.md). Custom builds discover/link internally; no mandatory user init/Register/import list.
- Start with [concepts.md](references/product/llm/datly-custom-component/references/concepts.md) for terminology, philosophy, base types, and authoring choices.
- Read [developer-mcp.md](references/product/llm/datly-custom-component/references/developer-mcp.md) before using a developer MCP server. Its operations are conceptual capabilities, not assumed tool names.
- Use [dql-grammar.md](references/product/llm/datly-custom-component/references/dql-grammar.md) and [dql.ebnf](references/product/llm/datly-custom-component/references/dql.ebnf) for DQL, directives, options, SQL/Velty boundaries, CAST, and tag customization.
- Use [tags-and-interfaces.md](references/product/llm/datly-custom-component/references/tags-and-interfaces.md) for Go shapes, binding tags, SQL mapping, predicates, validation, and public APIs.
- For JWT-based authorization, use the [explicit input and predicate pattern](references/product/llm/datly-custom-component/references/tags-and-interfaces.md#jwt-input-and-authorization-predicates); preserve original certificate/public-key verification and do not inject ambient claims.
- Read [custom-contract.md](references/product/llm/datly-custom-component/references/custom-contract.md) for this component's behavior and decisions.
- Adapt [custom-examples.md](references/product/llm/datly-custom-component/references/custom-examples.md); examples are patterns, not authorization to access a live database.
- For direct bytes, conditional finalizers, YAML docs, static/MCP resources and operational services, read [output-and-operations.md](references/product/llm/datly-custom-component/references/output-and-operations.md). Separate current APIs from pending authoring/integration contracts.
- Use [acceptance.md](references/product/llm/datly-custom-component/references/acceptance.md) to verify observable application behavior. Framework maintenance is outside this skill.
...
- For hook messaging or async work, use the injected-service and original-job-schema requirements in [custom-contract.md](references/product/llm/datly-custom-component/references/custom-contract.md); dry-run reads do not prove mutation side-effect isolation.
