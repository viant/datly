...
- For project init/build and deployment, read [project-build.md](references/product/llm/datly-writer/references/project-build.md). Custom builds discover/link internally; no mandatory user init/Register/import list.
- Start with [concepts.md](references/product/llm/datly-writer/references/concepts.md) for terminology, philosophy, base types, and authoring choices.
- Read [developer-mcp.md](references/product/llm/datly-writer/references/developer-mcp.md) before using a developer MCP server. Its operations are conceptual capabilities, not assumed tool names.
- Use [dql-grammar.md](references/product/llm/datly-writer/references/dql-grammar.md) and [dql.ebnf](references/product/llm/datly-writer/references/dql.ebnf) for DQL, directives, options, SQL/Velty boundaries, CAST, and tag customization.
- Use [tags-and-interfaces.md](references/product/llm/datly-writer/references/tags-and-interfaces.md) for Go shapes, binding tags, SQL mapping, predicates, validation, and public APIs.
- For JWT-based authorization, use the [explicit input and predicate pattern](references/product/llm/datly-writer/references/tags-and-interfaces.md#jwt-input-and-authorization-predicates); preserve original certificate/public-key verification and do not inject ambient claims.
- Read [writer-contract.md](references/product/llm/datly-writer/references/writer-contract.md) for this component's behavior and decisions.
- Adapt [writer-examples.md](references/product/llm/datly-writer/references/writer-examples.md); examples are patterns, not authorization to access a live database.
- For hook-injected message buses, commit-dependent publication or async job requests, read [mutation-messages.md](references/product/llm/datly-writer/references/mutation-messages.md).
- For stable-ID/FK gaps, async, telemetry, YAML docs, static/MCP resources and standalone status, read [availability-and-operations.md](references/product/llm/datly-writer/references/availability-and-operations.md). Separate current APIs from pending authoring/integration contracts.
- Use [acceptance.md](references/product/llm/datly-writer/references/acceptance.md) to verify observable application behavior. Framework maintenance is outside this skill.
