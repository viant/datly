...
The [greeting example](references/product/llm/datly-custom-component/references/custom-examples.md#typed-application-owned-handler)
shows complete Go source, explicit validator injection and error handling. The
[project fixture](references/product/datly/project/build/testdata/app/hooks/write.go.txt) shows a DML handler
...
The [forwarding example](references/product/llm/datly-custom-component/references/custom-examples.md#explicit-typed-forwarding-and-trusted-service-registration)
...
The [HTTP adapter](references/product/datly/gateway/http/handler.go.txt) recognizes `response.Response`
before ordinary encoding; [writeResponse](references/product/datly/gateway/http/response.go.txt) copies
...
through [documentation metadata](references/product/datly/doc/api-documentation.md#shared-yaml-dictionary-and-response-schemas).
