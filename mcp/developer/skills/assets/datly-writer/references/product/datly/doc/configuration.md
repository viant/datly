...
[All guides](references/product/datly/doc/README.md) · [Quickstart](references/product/datly/doc/quickstart.md) · [Standalone services](references/product/datly/standalone/SERVICES.md)
...
Use [project build](references/product/datly/doc/project-build.md) to discover and link application types and factories internally. Use `validate -h` for the actual
...
Inspect the [configuration type](references/product/datly/standalone/config/config.go.txt) and
[connector owner](references/product/datly/bootstrap/connector/config.go.txt) for exact fields. The
accepted CORS/API-key/warmup/OpenAPI/Observation settings and limits are also
listed in [standalone/SERVICES.md](references/product/datly/standalone/SERVICES.md).
...
Selected linked packages support [report/cube and composition](references/product/datly/doc/reports.md)
...
Programmatic and explicitly wired HTTP [async](references/product/datly/doc/async.md) are present in the
candidate. [Standalone async configuration](references/product/datly/standalone/ASYNC.md) wires `Jobs`
...
are candidate features with the bounded acceptance in [status](references/product/datly/doc/status.md).
