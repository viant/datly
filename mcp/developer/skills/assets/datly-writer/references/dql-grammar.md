# DQL authoring grammar

[Formal EBNF](dql.ebnf)
...
Use this reference to author declarations and view controls, then validate and generate pure Go with the matching Datly build. Syntax, installed capabilities and execution are separate checks; verify the connected build and its advertised capabilities.
...
Syntax fragment; adapt within the [complete reader contract](dql-grammar.md#a-shared-view-structure-for-readers-and-writers).
...
See [Reader predicates](product/llm/datly-reader/references/reader-predicates.md) for the predicate catalog and
...
writer roles are `setters`, `input_setters`, `indexes`, `entity_methods`, and
`types` (cross-package shape aliases). Role names are case-sensitive; a `type:`
role requires an exported Go identifier after the colon. Application edits
belong in create-once `lifecycle.go`. Standard writers emit
public setters and typed indexes but no component-private mutation/output phase
program; `output.go` owns the output contract. `$support_dest('type:CubeInput','cube.go')` selects the filename for a
...
syntax. See [Reader predicates](product/llm/datly-reader/references/reader-predicates.md) for the reader catalog.
