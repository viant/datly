...
[All guides](references/product/datly/doc/README.md) · [Quickstart](references/product/datly/doc/quickstart.md)

The local `v1` release copy provides `datly init` and `datly build`.
This guide does not imply a published Datly 1.0 CLI version.
The [build owner](references/product/datly/project/build/README.md) documents the complete API.
...
The README [setup script](references/product/datly/doc/examples/prepare-demo.py) carries the full selected
...
see [dependency tooling](references/product/datly/doc/status.md#dependency-tooling). Build preserves module choices and reports Go's errors. Add/remove
