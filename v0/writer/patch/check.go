...
func (p *service) removeNonExisting(ctx context.Context, manager dsc.Manager, conn dsc.Connection, view *data.View, index map[string][]interface{}, metrics *metric2.Metrics) error {
	if len(index) == 0 {
		return nil
	}
	...
	query := metric2.NewQuery(view.Name, &dsc.ParametrizedSQL{SQL: SQL, Values: SQLValues})
