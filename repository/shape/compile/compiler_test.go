...
`
	...
	require.NotNil(t, planned.Directives.Route)
	assert.Equal(t, "/v1/api/orders", planned.Directives.Route.URI)
	assert.Equal(t, []string{"GET", "POST", "PATCH"}, planned.Directives.Route.Methods)
	assert.Equal(t, "pkg.OrderJSON", planned.Directives.JSONMarshalType)
	assert.Equal(t, "pkg.OrderIn", planned.Directives.JSONUnmarshalType)
	assert.Equal(t, "pkg.OrderXMLIn", planned.Directives.XMLUnmarshalType)
	assert.Equal(t, "tabular", planned.Directives.Format)
	assert.Equal(t, "2006-01-02", planned.Directives.DateFormat)
	assert.Equal(t, "lc", planned.Directives.CaseFormat)
