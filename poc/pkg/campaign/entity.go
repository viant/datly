...
type Campaign struct {
	Id                       int                `sqlx:"name=ID,autoincrement,primaryKey" validate:"required"`
	...
	Creative                 []Creative         `typeName:"Creative" sqlx:"-" datly:"ralName=Creative,relColumn=ID,refTable=CI_CREATIVE,refColumn=CAMPAIGN_ID" sql:"SELECT ID, ADVERTISER_ID, CAMPAIGN_ID FROM CI_CREATIVE"`
	Audience                 []Audience         `typeName:"Audience" sqlx:"-" datly:"ralName=Audience,relColumn=ID,refTable=CI_AUDIENCE,refColumn=CAMPAIGN_ID" sql:"SELECT au.ID, au.TARGET, au.EXCLUSION, ad.CAMPAIGN_ID  FROM CI_AUDIENCE au           JOIN CI_AD_ORDER ad ON au.AD_ORDER_ID = ad.ID"`
