...
/*
 ? SELECT ARRAY_AGG(UserId) AS Values FROM  `/Acl` LIMIT 1
*/
	CurCampaignAclUserId *struct { Values []int }`datly:"kind=param,in=Campaign"` 
...
WHERE $criteria.In("ID", $CurCampaignAclUserId.Values)
*/
	CurAcl *Acl`datly:"kind=data_view,in=CurAcl"` 


/*
 ? SELECT ARRAY_AGG(UserId) AS Values FROM  `/Features` LIMIT 1
*/
	CurCampaignFeaturesUserId *struct { Values []int }`datly:"kind=param,in=Campaign"` 
...
WHERE $criteria.In("ID", $CurCampaignFeaturesUserId.Values)
*/
