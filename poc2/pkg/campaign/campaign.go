...
type Campaign struct {
	...
	Advertiser               *Advertiser         `typeName:"Advertiser" sqlx:"-" datly:"relName=Advertiser,relColumn=ADVERTISER_ID,relField=AdvertiserId,refTable=CI_ADVERTISER,refColumn=ID,refField=Id,refns=av" sql:"SELECT av.ID,                            av.CURRENCY_ID,                            DEFAULT_CHANNELS,                            AGENCY_ID,                            (SELECT ctz.IANA_TIMEZONE_STR FROM CI_TIME_ZONE ctz WHERE av.TIME_ZONE_ID = ctz.ID) AS IANA_TIMEZONE                     FROM CI_ADVERTISER av "`
	Flights                  []*Flights          `typeName:"Flights" sqlx:"-" datly:"relName=Flights,relColumn=ID,relField=Id,refTable=CI_CAMPAIGN_FLIGHT,refColumn=CAMPAIGN_ID,refField=CampaignId" sql:"SELECT * FROM CI_CAMPAIGN_FLIGHT"`
	CampaignCreative         []*CampaignCreative `typeName:"CampaignCreative" sqlx:"-" datly:"relName=CampaignCreative,relColumn=ID,relField=Id,refTable=CI_CAMPAIGN_CREATIVE,refColumn=CAMPAIGN_ID,refField=CampaignId" sql:"SELECT * FROM CI_CAMPAIGN_CREATIVE"`
	Creative                 []*Creative         `typeName:"Creative" sqlx:"-" datly:"relName=Creative,relColumn=ID,relField=Id,refTable=CI_CREATIVE,refColumn=CAMPAIGN_ID,refField=CampaignId" sql:"SELECT ID, ADVERTISER_ID, CAMPAIGN_ID FROM CI_CREATIVE c"`
	Audience                 []*Audience         `typeName:"Audience" sqlx:"-" datly:"relName=Audience,relColumn=ID,relField=Id,refTable=CI_AUDIENCE,refColumn=CAMPAIGN_ID,refField=CampaignId,refns=ad" sql:"SELECT au.ID, au.TARGET, au.EXCLUSION, ad.CAMPAIGN_ID  FROM CI_AUDIENCE au                                                                                     JOIN CI_AD_ORDER ad ON au.AD_ORDER_ID = ad.ID"`
	BidOverride              []*BidOverride      `typeName:"BidOverride" sqlx:"-" datly:"relName=BidOverride,relColumn=ID,relField=Id,refTable=CI_CAMPAIGN_BID_MULTIPLIER,refColumn=CAMPAIGN_ID,refField=CampaignId" sql:"SELECT * FROM CI_CAMPAIGN_BID_MULTIPLIER"`
	Event                    *Event              `typeName:"Event" sqlx:"-" datly:"relName=Event,relColumn=ID,relField=Id,refTable=CI_EVENT,refColumn=CAMPAIGN_ID,refField=CampaignId" sql:"SELECT * FROM CI_EVENT t"`
	Acl                      *Acl                `typeName:"Acl" sqlx:"-" datly:"relName=Acl,relColumn=CREATED_USER,relField=CreatedUser,refTable=CI_CONTACTS,refColumn=ID,refField=UserId" sql:"     SELECT ID USER_ID,            ACCOUNT_ID,            HasUserRole(ID, 'ROLE_READ_ONLY') AS IS_READ_ONLY,            HasUserRole(ID, 'ROLE_BUSINESS_OWNER') AS IS_BUSINESS_OWNER,            HasUserRole(ID, 'ROLE_ADMIN_CREATION') AS IS_ADMIN_CREATION,            HasUserRole(ID, 'ROLE_ROLES_MANAGEMENT') AS IS_ROLES_MANAGEMENT,            HasUserRole(ID, 'EXPOSE_COMMISSION') AS CAN_EXPOSE_COMMISSION,            HasUserRole(ID, 'ROLE_ADELPHIC_INTERNAL') AS ROLE_ADELPHIC_INTERNAL,            HasAgencyRole(1, ID,  'AGENCY_OWNER') IS_AGENCY_OWNER,            HasAgencyRole(1, ID,  'AGENCY_CAMPAIGN_MEMBER') IS_AGENCY_CAMPAIGN_MEMBER,            HasAdvertiserRole(1, ID,'ADVERTISER_OWNER') IS_ADVERTISER_OWNER,            HasAdvertiserRole(1, ID,'CAMPAIGN_MEMBER') IS_CAMPAIGN_MEMBER     FROM CI_CONTACTS     WHERE 1=2 "`
	Features                 *Features           `typeName:"Features" sqlx:"-" datly:"relName=Features,relColumn=CREATED_USER,relField=CreatedUser,refTable=CI_CONTACTS,refColumn=ID,refField=UserId" sql:"SELECT                         ID USER_ID,                         HasAccountFeatureEnabled(ACCOUNT_ID, 'EXPOSE_DUAL_STATUS') AS DUAL_STATUS,                         HasAccountFeatureEnabled(ACCOUNT_ID, 'EXPOSE_HOUSEHOLD_IDENTIFIER') AS HOUSEHOLD_IDENTIFIER,                         HasAccountFeatureEnabled(ACCOUNT_ID, 'EXPOSE_CAMPAIGN_FLIGHTING') AS CAMPAIGN_FLIGHTING,                         HasAccountFeatureEnabled(ACCOUNT_ID, 'EXPOSE_CHANNELS_V2') AS EXPOSE_CHANNELS_V2,                         HasAccountFeatureEnabled(ACCOUNT_ID, 'EXPOSE_XDEVICE_FREQUENCY_CAPPING') AS XDEVICE_FREQUENCY_CAPPING                     FROM CI_CONTACTS                     WHERE 1=2 "`
...
type Advertiser struct {
	...
	IANA_TIMEZONE   *string        `sqlx:"name=(SELECT ctz.IANA_TIMEZONE_STR FROM CI_TIME_ZONE ctz WHERE av.TIME_ZONE_ID = ctz.ID)" json:",omitempty" validate:"omitempty"`
...
type Audience struct {
	...
	CampaignId *int         `sqlx:"name=CAMPAIGN_ID" json:",omitempty" validate:"omitempty"`
...
type Acl struct {
	UserId                    int     `sqlx:"name=ID,primaryKey" validate:"required"`
	AccountId                 int     `sqlx:"name=ACCOUNT_ID,refTable=CI_ACCOUNT,refColumn=ID" validate:"required"`
	IS_READ_ONLY              *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_BUSINESS_OWNER         *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_ADMIN_CREATION         *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_ROLES_MANAGEMENT       *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	CAN_EXPOSE_COMMISSION     *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	ROLE_ADELPHIC_INTERNAL    *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_AGENCY_OWNER           *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_AGENCY_CAMPAIGN_MEMBER *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_ADVERTISER_OWNER       *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	IS_CAMPAIGN_MEMBER        *int    `sqlx:"name=" json:",omitempty" validate:"omitempty"`
...
type AclHas struct {
	...
	IS_ADMIN_CREATION         bool
	IS_ROLES_MANAGEMENT       bool
	CAN_EXPOSE_COMMISSION     bool
	ROLE_ADELPHIC_INTERNAL    bool
	IS_AGENCY_OWNER           bool
	IS_AGENCY_CAMPAIGN_MEMBER bool
	IS_ADVERTISER_OWNER       bool
	IS_CAMPAIGN_MEMBER        bool
}

type Features struct {
	UserId                    int          `sqlx:"name=ID,primaryKey" validate:"required"`
	DUAL_STATUS               *int         `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	HOUSEHOLD_IDENTIFIER      *int         `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	CAMPAIGN_FLIGHTING        *int         `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	EXPOSE_CHANNELS_V2        *int         `sqlx:"name=" json:",omitempty" validate:"omitempty"`
	XDEVICE_FREQUENCY_CAPPING *int         `sqlx:"name=" json:",omitempty" validate:"omitempty"`
...
type FeaturesHas struct {
	UserId                    bool
	DUAL_STATUS               bool
	HOUSEHOLD_IDENTIFIER      bool
	CAMPAIGN_FLIGHTING        bool
	EXPOSE_CHANNELS_V2        bool
	XDEVICE_FREQUENCY_CAPPING bool
}
